// File: js/app.js
// Student: Adam Ashraf Rosol Ghanem (12326691)
  ...
  form.addEventListener("submit", function (event) {
    ...
    .then(data => {
        if (data.success){
            ...
            setStatus("falid to add task", true);
        }
    })
    .catch(() => {
...
function renderTask(task) {
  ...
  deleteBtn.addEventListener("click", function() {
    ...
    fetch(url)
     ...
     .catch(() => {
        setStatus("error deleting taskk", true);
     });
  });
  li.appendChild(span);
